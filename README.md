...
2. Navegue até a pasta `server` no terminal e execute `java -jar target/server-0.0.1-SNAPSHOT.jar`(se estiver usando Maven, execute `mvn spring-boot:run` na pasta raíz `server`)
3. Navegue até a pasta `front` no terminal e execute `ng serve`
